  ...
  setProgram(gl) {
    ...
    this.fragmentShader = `
      ...
      void main() {
        ...
        vec3 skyTop = vec3(0.15, 0.3, 0.6);
        vec3 skyMid = vec3(0.4, 0.5, 0.7);
        vec3 horizon = vec3(0.7, 0.5, 0.35);
        vec3 bottom = vec3(0.15, 0.1, 0.08);

        vec3 color;
        if (height > 0.3) {
          color = mix(skyMid, skyTop, smoothstep(0.3, 1.0, height));
        } else if (height > 0.0) {
          color = mix(horizon, skyMid, smoothstep(0.0, 0.3, height));
        } else {
          color = mix(horizon, bottom, smoothstep(0.0, -1.0, height));
        }
    ...
    if (!this.program) console.error("Could not compile shaders for Sphere");
  }
